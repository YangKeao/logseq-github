...
export class GithubClient {
  ...
  async listAllIssues(repo: string, query: string) {
    ...
    while (true) {
      ...
      if (response.search.edges.length < limit) {
        ...
        const lastEdge =
          response.search.edges[response.search.edges.length - 1];
  ...
  async list_recent_merged_pr_in_repo(
    username: string,
    repo: string,
    after: Date
  ) {
    ...
    while (true) {
      ...
      const recentMergedPr = response.search.edges
        .map((edge) => edge.node)
        .filter((node: any) => new Date(node.mergedAt) > after);
